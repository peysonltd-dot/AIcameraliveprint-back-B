...
(async()=>{
 ...
 const fake=require('./fake-cloud.cjs').fakeCloud();const instance=require('../server').createApp({cloud:fake.make()});await instance.ready;const {app}=instance;const server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));
 try{
  ...
  const post=(route,body)=>realFetch(base+route,{method:'POST',headers:{'content-type':'application/json',...(route.startsWith('/api/admin/')?{authorization:'Bearer '+login.token}:{})},body:JSON.stringify(body||{})});
  ...
  assert.equal(health.layeredComposite,false);assert.equal(health.pipelineVersion,'leonardo-chibi-print-v13');
  ...
  async function run(sceneId='digital-ark-green'){
   ...
   for(let i=0;i<100;i++){
    ...
    if(data.status!=='pending' && data.cloudStatus==='saved')return {...data,id:task.taskId};
  ...
  for(const scene of scenes){
   ...
   assert(payload.parameters.prompt.includes('super cute minimalist'));assert(payload.parameters.prompt.includes('uniform pure white #FFFFFF'));assert(!payload.parameters.prompt.includes('#FF00FF'));assert(payload.parameters.prompt.includes('AUTHORITATIVE OFFICIAL MASCOT'));
   assert(task.resultImageB.startsWith('https://mock-storage/'));const record=fake.records.get(task.id);const decoded=await sharp(fake.images.get(record.resultImageBFile.path)).raw().toBuffer({resolveWithObject:true});
  ...
  reject=true;const failed=await run();assert.equal(failed.status,'failed');assert.equal((await post('/api/choice/'+failed.id,{choice:'B'})).status,400,'missing image cannot be confirmed');assert.equal(requests.length,7,'no paid retries');reject=false;
  review=true;const pending=await run();assert.equal(pending.printStatusB,'review');assert.equal(fake.records.get(pending.id).resultImageBFile.mime,'image/jpeg');
  assert.equal((await post('/api/choice/'+pending.id,{choice:'B'})).status,200);
  assert.equal((await status(pending.id)).printStatusB,'review','guest confirmation must not approve print quality');
  ...
  console.log('PASS MOCK ONLY: six IPs, one GPT Image 2 LOW request per guest, no watercolor or paid retry, transparent margins, review can be previewed/confirmed without print approval, reprocess/replace/approve recovery without regeneration.');
 }finally{instance.close();server.close();global.fetch=realFetch;}
